...
async function train() {
  ...
  model = tf.sequential({
    layers: [
      tf.layers.flatten({ inputShape: mobilenet.outputs[0].shape.slice(1) }), // Match MobileNet output
      tf.layers.dense({ units: 128, activation: 'relu' }), // More neurons for better learning
      tf.layers.dense({ units: 64, activation: 'relu' }),
      tf.layers.dense({ units: 5, activation: 'softmax' }) // 5 classes: Rock, Paper, Scissors, Lizard, Spock
    ]
  });
